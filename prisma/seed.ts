/**
 * Development seed: one realistic dental clinic.
 *
 * Why it attaches to a REAL account rather than inventing one: `Profile.id` is
 * the Supabase Auth user id. A fabricated profile would produce data nobody can
 * sign in and see, which is worse than no seed — it looks like the app works
 * while every screen is empty for the actual user.
 *
 * So: sign up in the app first, then run this. It finds your auth user by email
 * and makes you the OWNER of the seeded clinic.
 *
 *   npm run db:seed -- you@example.com
 *   SEED_ADMIN_EMAIL=you@example.com npm run db:seed
 *
 * Re-runnable: it deletes and recreates the demo organization by slug, which
 * cascades to everything beneath it. It touches no other organization.
 *
...
const DEMO_SLUG = "bright-smile-dental";

if (process.env.NODE_ENV === "production") {
  console.error(
    "Refusing to seed: NODE_ENV is production. This script deletes and recreates\n" +
      `the "${DEMO_SLUG}" organization and is for development only.`,
  );
  process.exit(1);
}

const adminEmail = (process.argv[2] ?? process.env.SEED_ADMIN_EMAIL ?? "").trim();

if (!adminEmail) {
  console.error(
    "Usage: npm run db:seed -- you@example.com\n\n" +
      "Pass the email of an account that has already signed up, so the seeded\n" +
      "clinic belongs to a user who can actually sign in and see it.",
  );
  process.exit(1);
}
...
async function main() {
  ...
  console.log(`Seeding "${DEMO_SLUG}" for ${authUser.email} ...`);

  // Re-runnable: cascade removes every child record of the demo clinic only.
  ...
  const organization = await prisma.organization.create({
    data: {
      ...
      slug: DEMO_SLUG,
      type: "DENTAL_CLINIC",
      description: "Family dental practice — development sample data.",
      email: "hello@brightsmile.example",
      phone: "+91 98400 11223",
      addressLine: "2nd Floor, 14 MG Road",
      city: "Bengaluru",
      state: "Karnataka",
      country: "IN",
  ...
  for (const [index, appointment] of upcoming.entries()) {
    ...
    await prisma.patientEmail.create({
      data: {
        ...
        subject: "Your upcoming appointment at Bright Smile Dental",
        body:
